...
asv.plot <- ggplot(alphadt_watersed, aes(x = fraction, y = value, fill=fraction)) +
  geom_boxplot() + 
  ylab('ASVRichness') +
  xlab(primary) +
...
residual_obs.stat <- resid(obs.stat); F1_obs.stat <- fitted(obs.stat)
plot(F1_obs.stat, residual_obs.stat, xlab = "Fitted Values", ylab = "Normalized residuals"); abline(h = 0, lty = 2) 
summary(obs.stat)
anova(obs.stat)
hist(residual_obs.stat) 
qqnorm(residual_obs.stat)
shapiro.test(residual_obs.stat)
LeveneTest(residual_obs.stat ~ fraction*mountain_range, data=alphadt_watersed)
...
vegan_matrix_sedwater <- vegan_otu(water_sediment_r)

water_sed_permanova <-adonis(vegan_matrix_sedwater ~ mountain_range*fraction, permutations=999, data = alphadt_watersed)

alphadt_watersed$all <- paste(alphadt_watersed$mountain_range, alphadt_watersed$fraction, sep="_")

pa_res <- pairwise.adonis(vegan_matrix_sedwater, alphadt_watersed$all, sim.method = 'bray',  p.adjust.m = 'holm', options(max.print=10000))
...
##Metacoder - differential abundance analyses
